...
from .xbwPMFSNetEdgeAux import xbwPMFSNetEdgeAux
...
from .PMFSNet_MobileNet import PMFSNet_MobileNet
...
def get_model_optimizer_lr_scheduler(opt):
    ...
    if opt["dataset_name"] == "3D-CBCT-Tooth" or opt["dataset_name"] == "Kfold-3D-CBCT-Tooth":
        ...
        elif opt["model_name"] == "PMFSNet_MobileNet":
             model = PMFSNet_MobileNet(
             in_channels=opt["in_channels"],
             out_channels=opt["classes"],
             dim=opt["dimension"],
            scaling_version=opt["scaling_version"]
                                                 )
        elif opt["model_name"] == "MSA_MobileNetV2_UNet":
            model = MSA_MobileNetV2_UNet(in_channels=opt["in_channels"], out_channels=opt["classes"])
        ...
        elif opt["model_name"] == "xbwPMFSNetEdgeAux":
            model = xbwPMFSNetEdgeAux(in_channels=opt["in_channels"], out_channels=opt["classes"], dim=opt["dimension"], scaling_version=opt["scaling_version"])
    ...
    if opt["dataset_name"] == "3D-CBCT-Tooth" or opt["dataset_name"] == "Kfold-3D-CBCT-Tooth":
        if opt["model_name"] == "DenseVNet":
            ...
            model = UNet3D(in_channels=opt["in_channels"], out_channels=opt["classes"], is_segmentation=False)
              ...
              model = PMFSNet_MobileNet(in_channels=opt["in_channels"], out_channels=opt["classes"])
            ...
            model = VNet(in_channels=opt["in_channels"], classes=opt["classes"])
             ...
             model = PMFSNet_MobileNet(in_channels=opt["in_channels"], out_channels=opt["classes"],
                              dim=opt["dimension"], scaling_version=opt["scaling_version"])
